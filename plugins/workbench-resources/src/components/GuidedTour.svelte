<script lang="ts">
  ...
  const tourCardActions = new Set<TourCardAction>(['start', 'next', 'previous', 'skipTour'])
  ...
  function startWhenReady (): void {
    ...
    phase = getGuidedTourPhase(progress?.activatedOn, progress?.completedOn, progress?.skippedOn)
    if (localProgress?.pendingSync === true) void syncLocalProgress()
  ...
  function applyRemoteProgress (preference: GuidedTourPreference): void {
    ...
    phase = getGuidedTourPhase(preference.activatedOn, preference.completedOn, preference.skippedOn)
  ...
  function storeRemoteProgress (preference: GuidedTourPreference): GuidedTourLocalProgress {
    const progress = {
      ...
      skippedOn: preference.skippedOn,
      pendingSync: false,
      updatedOn: Date.now()
    }
  ...
  function saveProgress (currentStep: number, activated = false, outcome?: 'completed' | 'skipped'): void {
    const now = Date.now()
    localProgress = {
      currentStep,
      activatedOn: localProgress?.activatedOn ?? savedPreference?.activatedOn ?? (activated ? now : undefined),
      completedOn: localProgress?.completedOn ?? savedPreference?.completedOn ?? (outcome === 'completed' ? now : undefined),
      skippedOn: localProgress?.skippedOn ?? savedPreference?.skippedOn ?? (outcome === 'skipped' ? now : undefined),
      pendingSync: true,
      updatedOn: now
    }
    ...
    syncPromise = (async () => {
      try {
        ...
        await client.updateDoc(guidedTourPreferenceClass, core.space.Workspace, id, {
          ...
          ...(snapshot.skippedOn !== undefined ? { skippedOn: snapshot.skippedOn } : {})
        })
  ...
  async function next (): Promise<void> {
    ...
    if (panelStepIndex === steps.length - 1) {
      saveProgress(panelStepIndex, true, 'completed')
      closeTourPanel()
      closeDemonstration()
      clearTarget()
      active = false
  ...
  /** Records an explicit opt-out locally before closing the tutorial. */
  function skipTour (): void {
    if (saving) return
    saveProgress(panelStepIndex, true, 'skipped')
    closeTourPanel()
    closeDemonstration()
    clearTarget()
    active = false
